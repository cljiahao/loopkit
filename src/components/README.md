...
- `elevated-card.tsx` — `ElevatedCard`: shared polished-card primitive (rounded corners, soft two-layer lifted shadow, renders as `div` or `section`) used across profile/dashboard/setup; deliberately not qkit's scalloped "kitchen ticket" `Ticket` theme
